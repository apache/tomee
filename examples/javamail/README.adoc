...
with the EmailService class and a WEB-INF/resources.xml declaring the
mail session. The session points to a GreenMail SMTP server the test
starts on a random port. testable = false runs the test methods as a
