...
Dentro da aplicação, há um endpoint que te vai da um histograma do clima das mais recentes temperaturas na cidade de Nova York.
...
As métricas do MicroProfile tem uma função que permite criar um histogram de dados.

Para utilizar esta função, injetamos um objeto `MetricRegistry`, registrar o histogram e 
Para utilizar esta función, injecta un objeto `MetricRegistry`, registra el Histograma, e adiciona dados ao histograma, como mostrado abaixo.
...
registre-se em mais de um local. Não se aplica ao `@Histogram`.
