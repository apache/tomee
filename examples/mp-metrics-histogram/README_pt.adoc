= Métricas com MicroProfile `@Histogram`
:index-group: MicroProfile
:jbake-type: page
:jbake-status: published

Este é um exemplo sobre como utilizar as métricas do MicroProfile no TomEE.
...
Dentro da aplicação, há um endpoint que vai te dar um histograma do clima das mais recentes temperaturas na cidade de Nova York.
...
As métricas do MicroProfile tem uma função que permite criar um histograma de dados.

Para utilizar esta função, injetamos um objeto `MetricRegistry`, registramos um `Histogram` e adicionamos dados ao histograma como mostrado abaixo:
...
Existem algumas configurações definidas na anotação `@Histogram`:
...
registrado em mais de um local. Não se aplica a gauges ou histogramas.
...
Uma métrica terá metadados para fornecer informação sobre ela como `displayName`, `description`, `tags`, etc.
...
28-May-2020 22:16:46.621 INFORMAÇÕES [main] org.apache.openejb.OpenEJB$Instance.<init> Copyright 1999-2024 (C) Apache OpenEJB Project, All Rights Reserved.
