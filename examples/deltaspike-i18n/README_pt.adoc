...
Esté exemplo mostra como usar o DeltaSpike I18n.
...
Criamos uma interface com um método para cada mensagem de tipo seguro. A interface deve ser anotatada com `@MessageBundle` e para cada mensagem é necessario ser anotada com `@MessageTemplate`:
...
Agora você pode injeta a interface e recuperar a mensagem de tipo seguro:
