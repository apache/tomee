= Utilitários de Concorrência para o Java EE
...
Além disso, em um objeto de thread não gerenciado, o contêiner não pode garantir que outros serviços da plataforma JEE funcionem corretamente. Por esses motivos, é recomendável o uso threads gerenciados sempre que a necessidade surgir.
...
Um `ManagedExecutorService` é um objeto que permite ao desenvolvedor do aplicativo enviem tarefas de forma assíncrona. As tarefas são executadas em threads que são gerenciadas pelo contêiner.
...
O método `asyncTask` simula uma execução bem sucedida, enquanti o `asyncTaskWithException` simula uma execução que vai lançar uma exceção.
...
Um `ManagedThreadFactory` é um objeto que permite aos  desenvolvedores criar threads gerenciados por contêiner.
...
O método `asyncTask` apenas cria uma thread gerenciada (usando o `ManagedThreadFactory` injetado) em seguida, o inicia.
O método `asyncHangingTask` também cria uma thread gerenciada, o inicia, mas também o para.
