...
https://eclipse-ee4j.github.io/jersey/[Jersey] will be used to create the web service.
...
Jersey is used instead of the CXF based JAX-RS implementation shipped with TomEE, so the webapp disables the latter
in `webapp/WEB-INF/application.properties`:

[source,properties]
----
openejb.jaxrs.on = false
----

EclipseLink is provided by the TomEE Plume distribution, which is also the one the test runs on.
