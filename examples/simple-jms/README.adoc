...
This example demostrate how to set up a ``CustomJmsService`` in order to produce and consume a JSM ```Message``.

= The Code

== The JMS service: ``Message``, ``Queue``, MessageProducer``, ``MessageConsumer``

Here we have a simple REST, we annotate the class with ``@Path("/message")`` to indicate the route corresponding to class ``CustomJmsService``. So we define ``sendMessage()`` as ``@POST``  and ``receiveMessage()`` as ``@GET`` for this ``/message`` route. 
...
Finally, interacting with intances of ``Connection``, ``Session``, ``Queue`` you can see instances of ``MessageProducer`` and ``MessageConsumer``, reponsible of writing and reading to/from the ``Queue`` repectively.
...
The test is trivial. The idea is first to POST a message to the service. This is done using intances of ``ClientBuilder`` and ``WebTarget``.

Then, similar to the POST, we perform a GET request that consumes the previously posted messsage.
