...
Classe jaxrs simples com um GET e um POST endpoint
...
Classe serializador de objetos simples que adiciona `modified 'durante a serialização
...
A execução do exemplo pode ser feita no maven com um simples comando `mvn clean install', executado no diretório `jsonb-custom-serializer'.
