...
O `org.apache.openejb.junit.ApplicationComposer` é o corredor de testes JUnit modelado de acordo com a forma como fizemos testes internos no OpenEJB por anos (desde 2006).
...
Com o `ApplicationComposer` você pode fazer testes idênticos ao OpenEJB que usa internamente, mas com dependência limitada do próprio OpenEJB.  A principal dependência está no código
