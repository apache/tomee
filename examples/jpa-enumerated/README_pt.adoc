...
Às vezes pode ser desejável ter um tipo `enum` do Java para representar uma coluna específica em um banco de dados. O JPA oferece suporte à conversão dos dados de um banco de dados para ou a partir de um tipo `enum` do Java por meio da anotação  `@javax.persistence.Enumerated`.
