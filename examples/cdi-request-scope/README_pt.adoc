...
Uma instancia de `Soup` sera criada nesse método e ela sera compartilhada em toda a requisição com o 
...
sera criada apenas uma vez para cada request e sera compartilhada por todos os beans que a injetam.
...
Na saida voce pode ver que existem duas instancias de `Soup` criadas - uma para cada requisição.
