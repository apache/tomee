:index-group: Other Features :jbake-type: page :jbake-status:
status=published = Bean Validation Design by Contract
...
OpenEJB provides an interceptor which allows you to use this feature to
do design by contract.
...
TomEE and OpenEJB do not provide anymore
`BeanValidationAppendixInterceptor` since Bean Validation 1.1 does it
(with a slighly different usage but the exact same feature).

So basically you don’t need to configure anything to use it. # Errors

If a parameter is not validated an exception is thrown, it is an
EJBException wrapping a ConstraintViolationException:
