...
/**
 * Pre-populates the TLD cache with the container's own taglibs (myfaces, jstl) so they don't have to
 * be parsed on every deployment; see {@code TomEEJarScanner.TomEEFilter} which skips those jars.
 * <p>
 * The bulk of {@link #populateMyfacesAndJstlData()} was dumped once from a running server by the
 * {@code sysout()} method kept commented out at the bottom of this file, then pasted in. It is not
 * generated by the build and is maintained by hand -- note that {@code sysout()} itself is stale: it
...
public class TomEETldScanner extends TldScanner {
    ...
    /**
     * TOMEE-4651: Jakarta Tags 3.0 renamed the JSTL taglib URIs from the legacy
     * {@code http://java.sun.com/jsp/jstl/*} form to the short {@code jakarta.tags.*} form.
     * <p>
     * The taglibs-standard implementation we shade in {@code deps/taglibs-shade} predates that rename:
     * the shade only rewrites bytecode ({@code javax.servlet} to {@code jakarta.servlet}), it does not
     * touch the TLD descriptors, so its TLDs still declare the legacy URIs only. Without this mapping
     * every {@code <%@ taglib uri="jakarta.tags.core" %>} fails at JSP compile time.
     * <p>
    ...
    static {
        JAKARTA_TAGS_URI_ALIASES.put("jakarta.tags.core", "http://java.sun.com/jsp/jstl/core");
        JAKARTA_TAGS_URI_ALIASES.put("jakarta.tags.fmt", "http://java.sun.com/jsp/jstl/fmt");
    ...
    /**
     * Registers the Jakarta Tags 3.0 URIs as additional keys for the TLD resources that
    ...
    private static void aliasJakartaTagsUris() {
        for (final Map.Entry<String, String> alias : JAKARTA_TAGS_URI_ALIASES.entrySet()) {
            ...
            // reuse the very same instance: scanPlatform() tells myfaces and jstl entries apart by
            // reference identity on the resource path's URL, and TLD_RESOURCE_TAG_LIB is keyed on it
            URI_TLD_RESOURCE.put(alias.getKey(), path);
        }

        permitJakartaTagsUris();
    }

    /**
     * The {@code permittedTaglibs} TLV restricts pages to a fixed list of taglib URIs, and the shaded
     * TLD only lists the legacy spellings. Adds the Jakarta Tags equivalents so a page guarded by that
     * validator may use either spelling.
     */
    private static void permitJakartaTagsUris() {
        final TldResourcePath path = URI_TLD_RESOURCE.get("http://jakarta.apache.org/taglibs/standard/permittedTaglibs");
        if (path == null) {
            return;
        }
