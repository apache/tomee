...
public class ManualFilterTest {
    ...
    public void checkItIsDeployed() throws Exception {
        try (final TomEEApplicationComposer runner = new TomEEApplicationComposer(ManualFilterTest.class)) {
            assertEquals("Filter = true", IO.slurp(new URL("http://localhost:" + runner.getPort() + "/ManualFilterTest")));
        }
    }
}
