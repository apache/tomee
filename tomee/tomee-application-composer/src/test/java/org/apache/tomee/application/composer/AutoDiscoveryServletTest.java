...
public class AutoDiscoveryServletTest {
    ...
    public void checkItIsDeployed() throws Exception {
        try (final TomEEApplicationComposer runner = new TomEEApplicationComposer(AutoDiscoveryServletTest.class)) {
            assertEquals("Init = true", IO.slurp(new URL("http://localhost:" + runner.getPort() + "/ServletTest")));
        }
    }
}
