...
public class ManualServletTest {
    ...
    public void checkItIsDeployed() throws Exception {
        try (final TomEEApplicationComposer runner = new TomEEApplicationComposer(ManualServletTest.class)) {
            assertEquals("TheServlet = true", IO.slurp(new URL("http://localhost:" + runner.getPort() + "/ManualServletTest")));
        }
    }
}
