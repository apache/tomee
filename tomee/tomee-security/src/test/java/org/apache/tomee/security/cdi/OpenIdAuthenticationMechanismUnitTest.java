...
public class OpenIdAuthenticationMechanismUnitTest {
    ...
    @Test
    public void basicAuthHeaderUrlEncodesClientCredentialsWithReservedChars() throws Exception {
        // RFC 6749 §2.3.1: client_id and client_secret MUST be application/x-www-form-urlencoded
        // before being joined with ':' and base64-encoded for client_secret_basic. Without this,
        // credentials containing reserved characters (':', '+', '%', ' ', non-ASCII) get split
        // by the receiver on the wrong ':' or decoded to unintended bytes, and Keycloak/Connect2id
        // reject them as malformed Basic credentials. Matches Nimbus oauth2-oidc-sdk
        ...
        try {
            // A client_id containing ':' would otherwise be split incorrectly by the AS, and a
            // client_secret with '+'/'%'/' '/non-ASCII would be misdecoded.
            when(definition.clientId()).thenReturn("client:with:colons");
            when(definition.clientSecret()).thenReturn("p@ss w0rd+ünïcödé/=&");
            when(definition.providerMetadata().tokenEndpoint())
                    .thenReturn("http://127.0.0.1:" + server.getAddress().getPort() + "/token");
            when(identityStoreHandler.validate(any()))
                    .thenReturn(new CredentialValidationResult("caller", Set.of("users")));
            ...
            authenticationMechanism.performAuthentication(request, response, messageContext);

            String header = capturedAuthHeader.get();
            assertNotNull("Authorization header expected when defaulting to client_secret_basic", header);
            assertTrue(header.startsWith("Basic "));

            // Decode and split on the ONLY ':' that isn't form-encoded.
            String decoded = new String(Base64.getDecoder().decode(header.substring("Basic ".length())),
                    StandardCharsets.UTF_8);
            int sep = decoded.indexOf(':');
            assertTrue("Decoded credential string must contain a separator colon", sep > 0);
            String encodedId = decoded.substring(0, sep);
            String encodedSecret = decoded.substring(sep + 1);

            // The id contained ':' twice -- those MUST have been form-encoded so the receiver
            // can recover the original.
            assertEquals("client%3Awith%3Acolons", encodedId);
            // The secret contains '+', ' ', '%', '=', '&', '/', non-ASCII. URL-decoding must
            // round-trip it back to the original value.
            assertEquals("p@ss w0rd+ünïcödé/=&",
                    URLDecoder.decode(encodedSecret, StandardCharsets.UTF_8));
            // And the id must round-trip too.
            assertEquals("client:with:colons",
