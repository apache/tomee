...
public class CmpFieldGetter implements InstanceOperation {
    private final CmpField field;

    public CmpFieldGetter(CmpField field) {
        this.field = field;
    }

    public Object invokeInstance(CmpInstanceContext ctx, Object[] args) {
        if (args.length != 0) {
            throw new IllegalArgumentException("CmpFieldGetter must not be passed any arguments:" +
