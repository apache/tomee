...
The application runs on the ApplicationComposer, which has no HTTP server. Remote clients reach its EJBs over ejbd
(`@EnableServices("ejbd")`), JAX-RS and JAX-WS endpoints are not served.
