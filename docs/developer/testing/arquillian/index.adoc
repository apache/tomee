...
- openejb-embedded: a plain embedded OpenEJB supporting most of EE features, without an HTTP server (no servlet, JAX-RS or JAX-WS endpoints)
...
| stopHost       | which port to use to shut down TomEE (port on Server configuration)
| stopCommand       | which command to use to shut down TomEE
