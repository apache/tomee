...
=== Services
`@EnableServices` starts the listed server services, for instance `ejbd` for remote EJB clients:
...
The ApplicationComposer has no HTTP server, so it doesn't deploy JAXRS or JAXWS endpoints. To test them over HTTP, use the TomEE Embedded runners (`tomee-embedded`), which support `@RandomPort("http")` to get a random HTTP port, or the Arquillian TomEE Embedded/Remote adapters.
...
=== @WebResource
Adds folders containing web resources to the web application. Only the TomEE Embedded runners serve them.
...
That's why `SingleApplicationComposerRunner` allows to just reuse the same instance across several test.
