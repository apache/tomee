...
openejb.ejbd.datasource-metadata

boolean (default: false)

Allow remote datasource lookup over EJBd, including HTTP/HTTPS. Enabling this
shares database connection details, including credentials, with clients able to
reach the endpoint. Applies server-wide, including datasource references; it
does not add per-user authorization. See link:ejbd-transport.html[Ejbd Transport]
for compatibility and configuration details.
...
override the host. Available for the ejbd service, number of thread to
manage requests
...
https://tomee.apache.org/embedded-configuration.html and
