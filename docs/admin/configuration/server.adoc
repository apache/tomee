...
|.bind, <service prefix>.port, <service prefix>.disabled, <service prefix>.threads	| host or IP, port, bool|override the host. Available for the ejbd service, number of thread to manage requests
...
|openejb.internal.beans.security.enabled	|boolean	|if true (default) the internal EJBs (Deployer, ConfigurationInfo, ...) can only be invoked by a caller in the tomee-admin or openejb-admin role
