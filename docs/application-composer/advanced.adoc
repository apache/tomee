...
JAXRS and JAXWS endpoints are served by TomEE. To test them over HTTP,
use TomEE Embedded (`tomee-embedded`) or the Arquillian TomEE
Embedded/Remote adapters, which support `@RandomPort("http")` to get a
random HTTP port.
